...
public partial class Tax_Home : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            else if (User_Role_Id == "1")
            {
                ...
                list_itemScore.Attributes.Add("class", "active");
                //li_queue.Visible = true;
                //divque.Visible = true;
            }

            GetOrder_Type_Wise_Data();

            Get_Tax_Status_Wise_Data();

          Get_Tax_Installment_Wise_Data();
        }
    ...
    protected void Btn_View_Click(object sender, EventArgs e)
    {
        ...
        if (ddl_Product_Type.SelectedIndex > 0 && ddl_ClientName.SelectedIndex>0 && ddl_SubProcess.SelectedIndex>0)
        {
            Session["Order_Type"] = "Web/Call";
            Session["client_Id"] = int.Parse(ddl_ClientName.SelectedValue.ToString());
            Session["subProcess_id"] = int.Parse(ddl_SubProcess.SelectedValue.ToString());
            Session["order_id"] = ViewState["Orderid"].ToString();
    ...
    [WebMethod]
    public static List<ChartType> GetOrder_Type_Wise_Data()
    {
    ...
    [WebMethod]
    public static List<ChartType> Get_Tax_Status_Wise_Data()
    {
        string constr = ConfigurationManager.ConnectionStrings["TaxManagementConnectionString"].ConnectionString;
        DataTable dt = new DataTable();
        using (SqlConnection con = new SqlConnection(constr))
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT  dbo.Tbl_Tax_Status.Tax_Status,count(dbo.Tbl_Orders.Order_ID) as Total_Orders FROM  dbo.Tbl_Orders INNER JOIN dbo.Tbl_Orders_Tax_Entry ON dbo.Tbl_Orders.Order_ID = dbo.Tbl_Orders_Tax_Entry.Order_Id INNER JOIN dbo.Tbl_Tax_Status ON dbo.Tbl_Orders_Tax_Entry.Tax_Status_Id = dbo.Tbl_Tax_Status.Tax_Status_Id where Tbl_Orders.Status='True' and Tbl_Tax_Status.Status='True' and Tbl_Orders_Tax_Entry.Status='True' group by Tbl_Tax_Status.Tax_Status ", con);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            da.Fill(dt);
            con.Close();
        }
    ...
    public static List<ChartType> Get_Tax_Installment_Wise_Data()
    {
        string constr = ConfigurationManager.ConnectionStrings["TaxManagementConnectionString"].ConnectionString;
        DataTable dt = new DataTable();
        using (SqlConnection con = new SqlConnection(constr))
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT UPPER(dbo.Tbl_Tax_Period.Tax_Period) as Tax_Installment,COUNT(dbo.Tbl_Orders.Order_ID) AS Total_Orders FROM dbo.Tbl_Orders INNER JOIN dbo.Tbl_Orders_Tax_Entry ON dbo.Tbl_Orders.Order_ID = dbo.Tbl_Orders_Tax_Entry.Order_Id INNER JOIN dbo.Tbl_Tax_Period ON dbo.Tbl_Orders_Tax_Entry.Tax_Period_Id = dbo.Tbl_Tax_Period.Tax_Period_Id WHERE        (dbo.Tbl_Orders.Status = 'True') AND (dbo.Tbl_Orders_Tax_Entry.Status = 'True') GROUP BY dbo.Tbl_Tax_Period.Tax_Period", con);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            da.Fill(dt);
            con.Close();
        }
