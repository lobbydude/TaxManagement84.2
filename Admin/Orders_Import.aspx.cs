...
public partial class Admin_Orders_Import : System.Web.UI.Page
{
    ...
    int GET_CLIENT, GET_SUBPROCESS, GET_ORDER_STATUS, GET_ORDER_TYPE,GET_PRODUCT_TYPE, GET_CLIENT_STATE, GET_BARROWER_STATE, GET_CLIENT_COUNTY, GET_BARROWER_COUNTY, CHECK_ORDER;
    ...
    protected void ImportToGrid(String path)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            htinsertrec.Add("@Product_Type", dt.Rows[i]["Product_Type"].ToString());
    ...
    protected void btn_Save_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in grd_order.Rows)
        {
            ...
            if (chk.Checked == true)
            {
                ...
                //get_Order_Type
                Hashtable htProductType = new Hashtable();
                DataTable dtProductType = new System.Data.DataTable();
                htProductType.Add("@Trans", "GET_PRODUCT_TYPE");
                htProductType.Add("@Product_Type", lblProduct_Type.Text);
                dtProductType = dataaccess.ExecuteSP("Sp_Order_Get_Details", htProductType);
                if (dtProductType.Rows.Count > 0)
                {

                    GET_PRODUCT_TYPE = int.Parse(dtProductType.Rows[0]["Product_Type_Id"].ToString());
                }
                else
                {
             ...
             //   GET_SUBPROCESS = Subprocess_id;
                ...
                htinsertrec.Add("@Product_Type", GET_PRODUCT_TYPE);
